...
def admin_login():
    if request.method == 'POST':
        if request.form.get('user') == 'a' and request.form.get('pass') == '1':
...
def all_products():
    ...
    # Sorting logic
    sort_logic = [('_id', -1)] # Default: Newest
    ...
    # Fetch categories for the filter bar (unique categories from DB)
    ...
    products = list(db.products.find(query).sort(sort_logic).skip(skip).limit(per_page))
